...
class MSFmultiSelect{
  constructor(select, settings={}) {
    ...
    this.defultSettings={
      ...
      //height:30,
      appendTo:'body',
      className:'uk-textarea uk-height-max-small zm-textarea-multiselect'
    }
  ...
  create(){
    ...
    ul.className='msf_multiselect uk-width-1-1 uk-background-default uk-overflow-auto uk-height-max-small';
    ...
    for(var i=0,len=this.select.children.length; i<len; i++){
      ...
      if(this.select.children[i].selected){
        li.className='active';
        input.checked=true;
        display+=(display) ? "\r\n"+this.select.children[i].innerText : this.select.children[i].innerText;
      }else{
        li.className='';
        input.checked=false;
      }
